...
class PlaceOrderCommand:
    ...
    def execute(request, shipping_data):
        ...
        # Get payment type from shipping data
        payment_type = shipping_data.get("payment_type", "paystack").lower()
        
        # Route to appropriate payment handler
        if payment_type == "paystack":
            checkout_link = paystack_initiate(request, user=request.user, cart_id=cart.id, data=shipping_data)
        elif payment_type == "monnify":
            # TODO: Implement Monnify payment logic
            checkout_link = paystack_initiate(request, user=request.user, cart_id=cart.id, data=shipping_data)
        elif payment_type == "flutterwave":
            checkout_link = flutterwave_initiate(request, user=request.user, cart_id=cart.id, data=shipping_data)
        else:
            op.fail(f"Unknown payment type: {payment_type}")
            return BaseResultWithData(
                data=None,
                status_code=HTTPStatus.BAD_REQUEST,
